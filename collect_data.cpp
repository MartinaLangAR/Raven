...
int main() {
    ...
    input_stream.open("input_cam2.csv", std::ios_base::app);
    label_stream.open("labels_cam2.csv", std::ios_base::app);
	    ...
	    if (faces.size() > 0){
			...
			//save match for dataset
		       ...
		       while (dist.second != 0 && limits_ok.second){
			       ...
			       if (!limits_ok.second){
				       percentage_back = (dist.second = -1) ? 0.0: 100.0; 
		       ...
		       while (dist.first != 0 && limits_ok.first){
			       ...
			       //adjust servo at front for a little bit
 			       percentage_front += dist.first * std::abs(moving_aim.center_x - 300) / 6; // <-------- hard coded with experimentally found values 
			       limits_ok = servos.move_to_percentage(percentage_front, percentage_back);
			       ...
			       if (!limits_ok.first){
				       ...
				       percentage_front = (dist.first = -1) ? 0.0: 100.0; 
