...
void FFN::initFFN(int nb_inputs_neurons, int nb_hidden_neurons, int nb_outputs_neurons){
    ...
    this->add_layer(new Layer(2, nb_hidden_neurons, this)); //<---added later on
    this->add_layer(new Layer(3, nb_outputs_neurons, this));
}
...
void FFN::test(std::vector<std::vector<float>> Xtest, std::vector<std::vector<float>> Tt){
...
void FFN::train_by_error(std::vector<std::vector<float>> inputs, std::vector<std::vector<float>> targets, float target_error){
    float error;
    do{
        error = 0;
...
void FFN::train_by_iteration(std::vector<std::vector<float>> inputs, std::vector<std::vector<float>> targets, int target_iteration){
    int ite = 0;
    do{
        ite++;
        //error = 0;
...
std::vector<float> FFN::get_ffn_outputs(){
    return this->get_layer_at(this->get_nb_layers()-1)->get_outputs();
}

vector<float> FFN::out(vector<float> inputs){
    this->sim(inputs);
    return this->get_ffn_outputs();
    };
    
vector<vector<vector<float>>> FFN::get_weights(){
    vector<vector<vector<float>>> weights;
    for (int layer_idx = 1; layer_idx < this->get_nb_layers(); layer_idx++){
		Layer* curr_layer = this->get_layer_at(layer_idx);
		weights.push_back(curr_layer->get_weights());
	}
    return weights;
}

void FFN::set_weights(vector<vector<vector<float>>> new_weights){
    for (int layer_idx = 1; layer_idx < this->get_nb_layers(); layer_idx++){
		Layer* curr_layer = this->get_layer_at(layer_idx);
		curr_layer->set_weights(new_weights[layer_idx-1]); 
	}
}
