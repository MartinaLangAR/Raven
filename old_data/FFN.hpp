...
class FFN{
    ...
    public:
        ...
        vector<vector<vector<float>>> get_weights();
        void set_weights(vector<vector<vector<float>>> new_weights);
        
        void about();
        
        vector<float> get_ffn_outputs();
        vector<float> out(vector<float> inputs);
