...
void Layer::set_weights(vector<vector<float>> new_weights) {
        if (weights.size() != new_weights.size()){ 
            std::cout << "Layers " << indice << " num of weights "<< new_weights.size() << " does not match the number of neurons " << weights.size() << " !" << std::endl;
        }
        weights = new_weights;
    };
